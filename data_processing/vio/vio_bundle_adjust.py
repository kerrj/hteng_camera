...
v_ik is the observed ray rotated into world frame by the CURRENT pose, X_k
the landmark, c_i the camera position, d_ik a free per-observation scale.
At the optimal d_ik this residual's squared norm equals sin(theta) capped
at 1 (theta = angle between v_ik and X_k-c_i) -- bounded to [0,1], unlike
reprojection error, which blew up under random-ball init (needed since the
IMU/gravity priors alone don't fix scale/position). Rotation stays a free
...
class Point3Var(jaxls.Var[jax.Array], default_factory=lambda: jnp.zeros(3)):
    """3D landmark position, world frame (= frame-0 left-camera frame)."""
...
def positioning_cost(vals, pose_var, point_var, scale_var, ray_cam, rel_wxyz_xyz,
    ...
    T_wc = T_rel @ T_wl
    R_wc = T_wc.rotation()
    t_wc = T_wc.translation()
    cam_pos_world = -(R_wc.inverse() @ t_wc)
    ray_world = R_wc.inverse() @ ray_cam
    point_world = vals[point_var]
...
def main():
    ...
    def solve_round(pose_init, point_init, scale_init):
        ...
        if keep_edge.sum() > 0:
            costs.append(imu_rotation_cost(
        ...
        initial_vals = jaxls.VarValues.make([
            camera_vars.with_value(pose_init),
            point_vars.with_value(point_init),
            scale_vars.with_value(scale_init),
        ])

        t0 = time.time()
        problem = jaxls.LeastSquaresProblem(
            costs, [camera_vars, point_vars, scale_vars]).analyze()
        ...
        if args.profile_compile:
            ...
            probe_iters = min(args.max_iterations, 5)
    ...
    def angular_residuals(solution, obs_subset=None):
        ...
        pts = np.asarray(solution[Point3Var])
        q = poses[pose_ids[m]]
    ...
    np.savez(out_path,
              frame_idx=frame_idx,
              pose_wxyz_xyz=np.asarray(poses_opt.wxyz_xyz),
              points=np.asarray(points_opt),
