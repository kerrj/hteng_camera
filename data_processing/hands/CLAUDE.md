# data_processing/hands — working notes for Claude
...
context. Head/camera pose is a separate, parallel pipeline — see
`../vio/CLAUDE.md`. Data shared by both lives one level up — see
`../CLAUDE.md`.
...
(resolution, fisheye model, baseline, calib file layout) — it's the same data
both this pipeline and `../vio/` consume.
...
Output: `data_processing/hands/out/stereo/{left,right}/hands.jsonl` (same schema).
...
3. **Head pose** — separate pipeline, now underway in `../vio/` (see
   `../vio/CLAUDE.md`), not built here.
